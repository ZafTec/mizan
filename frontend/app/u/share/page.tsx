...
export default async function SharePage({
  ...
  if (!token) notFound();
  const profile = await serverApi<SocialProfileDto>(
    `/api/Social/share/${encodeURIComponent(token)}`,
    { requireAuth: false },
  );
  return (
    <main className="mx-auto max-w-2xl py-10">
      <div className="card grid gap-8 p-8 text-center sm:grid-cols-[12rem_1fr] sm:text-left">
        <Image
        ...
        />
        <div>
          <p className="eyebrow">Mizan training circle</p>
          <h1 className="mt-3 text-3xl font-semibold">{profile.displayName}</h1>
          <p className="mt-3 text-charcoal-blue-500">
            This profile shares workouts only with approved followers.
          </p>
          <div className="mt-6">
            <FollowRequestButton token={token} />
          </div>
        </div>
      </div>
    </main>
