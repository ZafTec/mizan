...
export default function WorkoutDashboard({
	...
	const restRemaining = useMemo(() => {
		...
		return Math.max(
			0,
			draft.restTimer.seconds - Math.floor((Math.max(clock, startedAt) - startedAt) / 1000),
		);
		...
		try {
			dispatch({
				...
				draft: draftFromTemplate(
					await clientApi<NextSessionDto>(`/api/WorkoutTemplates/${id}/next-session`),
				),
			});
			selectTab("log");
		} catch (error) {
		...
		<div className="space-y-6 lg:space-y-8" data-testid="workouts-page">
			...
			<header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
				<div>
					...
					<h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Workouts</h1>
