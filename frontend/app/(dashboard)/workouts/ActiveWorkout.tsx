...
export default function ActiveWorkout({
  ...
  return (
    <fieldset disabled={finishing} className="space-y-5">
      <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3 sm:grid-cols-3">
      ...
      <details className="border-b border-border pb-3">
        <summary className="min-h-8 text-sm text-muted-foreground">
          Bodyweight and set help
        </summary>
      ...
      {restRemaining !== null && (
        <div
          aria-live="polite"
          className="sticky top-0 z-20 flex items-center justify-between rounded-2xl border border-tuscan-sun-300 bg-tuscan-sun-50 px-4 py-3 dark:border-tuscan-sun-500/30 dark:bg-tuscan-sun-950/80"
        >
      ...
      {draft.exercises.length === 0 && <ExercisePicker dispatch={dispatch} />}
      ...
      {draft.exercises.length === 0 ? (
        ...
        draft.exercises.map((exercise, exerciseIndex) => (
          <article key={exercise.uid} className="border-t border-foreground/70">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-[2rem_1fr_1fr_2.75rem] gap-2 px-2 text-xs font-semibold text-muted-foreground">
                <span>Set</span>
                <span>Weight (kg)</span>
                <span>Reps</span>
                <span>Done</span>
              ...
              {exercise.sets.map((set, index) => (
                <div
                  key={set.uid}
                  className={`grid grid-cols-[2rem_1fr_1fr_2.75rem] items-center gap-2 border-b border-border px-2 py-3 ${set.completedAt ? "bg-brand-50 dark:bg-brand-950/40" : ""}`}
                >
                  <span className="text-center text-sm font-semibold text-muted-foreground">
                    {index + 1}
                  </span>
                  <input
                    aria-label={`Weight (kg) for set ${index + 1}`}
                  ...
                  <button
                    ...
                    className={`press-feedback flex h-11 w-11 items-center justify-center rounded-2xl ${set.completedAt ? "bg-brand-600 text-white" : "bg-charcoal-blue-100 text-muted-foreground dark:bg-charcoal-blue-900"}`}
                    onClick={() =>
                      dispatch({
                        type: "cycle-reps",
                        exerciseUid: exercise.uid,
      ...
      {draft.exercises.length > 0 && (
        <details className="border-t border-border py-3">
          <summary className="min-h-8 text-sm font-medium">
            Add an exercise
          </summary>
          <div className="mt-3">
      ...
      <button
        ...
        disabled={draft.exercises.length === 0 || finishing}
        onClick={onFinish}
      >
